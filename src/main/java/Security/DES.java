...
public class DES {
    ...
    private static String[] generateSubKeys(String keyBin) {
        ...
        String pc1=permute(keyBin,PC1);
        String left = pc1.substring(0,28);
        String right = pc1.substring(28,56);
    ...
    private String feistel(String R, String subKey) {
        ...
        for (int i = 0; i < 8; i++) {
            int start=i*6;
            int end=start+6;
            String sub=xored.substring(start,end);
            String row=String.valueOf(sub.charAt(0)) + sub.charAt(5);
            String column = sub.substring(1,5);
            int rowNumber=Integer.parseInt(row,2);
            int columnNumber=Integer.parseInt(column,2);
            String s=Integer.toBinaryString(SBOXES[i][rowNumber][columnNumber]);
            substituted.append(String.format("%4s",s).replace(' ','0'));
        }
